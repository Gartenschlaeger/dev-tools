...
export class TextDiffService {
    private diffParser: diff_match_patch;

    constructor() {
        this.diffParser = new diff_match_patch();
    }

    private isNil(val: unknown) {
        return val === undefined || val === null;
    }

    private isEmpty(val: string) {
        return this.isNil(val) || val.length === 0;
    }
                        ...
                        .forEach((line) => {
                            rightDiffRow = rows.find(
                                (row) =>
                                    !row.leftContent &&
                                    row.rightContent &&
                                    row.rightContent.lineNumber === lineLeft &&
                        ...
                        .forEach((line) => {
                            leftDiffRow = rows.find(
                                (row) =>
                                    row.leftContent &&
                                    !row.rightContent &&
                                    row.leftContent.lineNumber === lineRight &&
